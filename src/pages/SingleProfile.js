...
import { faEllipsisV, faExternalLinkAlt, faPen, faArrowRight, faArrowLeft,faChevronDown } from "@fortawesome/free-solid-svg-icons";
import "swiper/css";
      ...
      <div className="p-6 relative w-full">
        ...
        <div className="flex flex-col items-center">
          ...
          <p className="font-semibold">
            <a href="https://www.google.com/maps?q=Colorado,USA" target="_blank" rel="noopener noreferrer">
...
<div className="flex flex-col sm:flex-row sm:flex-wrap justify-center items-center gap-4 sm:gap-8 mt-6 relative overflow-visible">
  {["Skills", "Preferences", "Experience"].map((item, index) => (
    <div key={index} className="relative">
      <button
        className="w-40 h-12 px-6 py-2 border text-white rounded-lg flex items-center gap-2 sm:w-40 sm:h-12 sm:px-10 sm:py-3 hover:border-purple-500 transition"
      ...
      {/* Dropdown Menu */}
      {dropdownOpen === item && (
        <div className="absolute left-0 top-12 w-48 bg-[#3D3450] text-white rounded-lg shadow-lg sm:w-56 z-50">
          <ul className="py-2">
            <li className="px-4 py-2 hover:bg-[#BAA7FC2E] cursor-pointer">Option 1</li>
            <li className="px-4 py-2 hover:bg-[#BAA7FC2E] cursor-pointer">Option 2</li>
            <li className="px-4 py-2 hover:bg-[#BAA7FC2E] cursor-pointer">Option 3</li>
          </ul>
        </div>
      )}
    </div>
      ...
      <div className="w-full mt-10 px-10 lg:px-20">
        <div className="flex justify-between items-center mb-4">
          ...
          <div className="flex gap-2">
            <button onClick={() => handleSwipe(postsSwiperRef, "prev")} className="w-10 h-10 rounded-full hover:border-purple-500 border-2">
              <FontAwesomeIcon icon={faArrowLeft} />
            </button>
            <button onClick={() => handleSwipe(postsSwiperRef, "next")} className="w-10 h-10 rounded-full hover:border-purple-500 border-2">
        ...
        >
          {posts.map((post, index) => (
            <SwiperSlide key={index} className="bg-[#BAA7FC2E] rounded-lg shadow-md p-4 relative">
              ...
              <p className="text-xs text-white absolute bottom-2 right-2">{post.date}</p>
            </SwiperSlide>
          ))}
        </Swiper>
      </div>
